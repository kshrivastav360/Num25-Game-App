...
class HomeScreen2a extends ConsumerStatefulWidget {
  HomeScreen2a({super.key, this.name});
  late String? name;
  @override
  ConsumerState<HomeScreen2a> createState() => _HomeScreen2aState();
}

class _HomeScreen2aState extends ConsumerState<HomeScreen2a>
  ...
  Future<void> updateScore(int scr, int time) async {
    ...
    final url = Uri.parse(
        'http://77.37.44.61:8086/mpuzzle2/leader/update/$phNo/$scr/$time/${4}');
    print("time : $time, $level");
    try {
      ...
      if (response.statusCode == 200) {
        Navigator.push(
            context,
           PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) => Marks4(
                mark: scr,
              ),
              transitionsBuilder:
