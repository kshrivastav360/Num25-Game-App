...
class HomeScreen1a extends ConsumerStatefulWidget {
  const HomeScreen1a({super.key});
  // late String? name;
  // late String? phNo;
  @override
  ConsumerState<HomeScreen1a> createState() => _HomeScreen1aState();
}

class _HomeScreen1aState extends ConsumerState<HomeScreen1a> {
  ...
  Future<void> updateScore(int scr, int time) async {
    ...
    final url = Uri.parse(
        'http://77.37.44.61:8086/mpuzzle2/leader/update/$phNo/$scr/$time/${2}');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        Navigator.push(
            context,
            PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) => Marks2(
                mark: scr,
